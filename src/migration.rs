...
struct OutputDataset {
    ...
    #[serde(skip_serializing_if = "policy_is_empty")]
    policy: OutputPolicy,
}
...
fn policy_is_empty(policy: &OutputPolicy) -> bool {
    policy == &OutputPolicy::default()
}

pub fn convert_sanoid(source: &str, defaults: Option<&str>) -> Result<SanoidMigration> {
    let mut warnings = vec![
        "Sanoid evaluates preferred snapshot times in the host's local time; zsnap evaluates them in UTC, so review every migrated schedule before enabling zsnap"
            .to_owned(),
    ];
    ...
    let body = toml::to_string_pretty(&output).context("failed to render migrated TOML")?;
    let config_toml = format!(
        "# Generated by `zsnap migrate-sanoid`.\n\
         ...
         # Sanoid schedule fields use host local time; zsnap schedule fields use UTC. Review them.\n\n\
         {body}"
    );
    let parsed: Config =
        toml::from_str(&config_toml).context("generated zsnap TOML did not parse")?;
    parsed
        .validate()
...
"#,
        ...
        .unwrap();
        let config: Config = toml::from_str(&migration.config_toml).unwrap();
